...
SQL_TABLE_CALIBRATOR_DATA = f'CREATE TABLE  IF NOT EXISTS Calibrator_info ('\
                    f'`Key_id` INTEGER NOT NULL UNIQUE AUTO_INCREMENT,'\
                    f'Name TEXT,'\
                    f'SN TEXT,'\
                    f'Cal_Date TEXT,'\
                    f'Meas_Date TEXT,'\
...
def initialise_database( db_name):
    ...
    database = CDBManager(db_name) if "sqlite3" in DB_SYST else \
               CDBManager('utllafond','ELA.AP3saucats', host='10.41.33.97', port='5432')
...
class CDBManager():
    ...
    def get_db_date(self):
        self.connect()
        sqlcmd = "SELECT datetime('now','localtime');"
        cur = self.connector.execute(sqlcmd)
        res = cur.fetchone()
        return res[0]
    ...
    def retrive_giv_list(self):
        """ Get list of Giv identifier and there key_id """ 
        self.connect()
        sqlcmd = f"SELECT Key_id, Giv_id, Date_meas FROM GIV_ids, Measure_points "\
                  "WHERE Giv_id IS NOT 'None' AND Key_id =Giv_Ref;"
        cur = self.connector.execute(sqlcmd)
        self.giv_key = cur.fetchall()
    ...
    def register_giv(self, giv_id):
        ...
        self.connector.commit()
        self.giv_id = giv_id    # Memorisation of the GIV ID
        return self.get_giv_key(giv_id)



    def get_giv_key(self, giv_id):
        self.connect()
        sqlcmd = f"SELECT Key_id FROM GIV_ids WHERE GIV_id = '{giv_id}';"
        cur = self.connector.execute(sqlcmd)        
        try:
            # modif 26/08 (self.giv_key, self.giv_id) = cur.fetchone()
            self.giv_key = cur.fetchone()[0]  # Une seul colone
            print ("Giv key:{}".format(self.giv_key ))
        except:
            print (f"GIV {giv_id} not found.")
            self.giv_key = None
        return self.giv_key
    ...
    def register_giv_last_cal_date(self, date, giv_id):
        ...
        curs = self.cursor.execute(sqlcmd)
        self.connector.commit()
        sqlcmd = f" SELECT Key_id FROM RecDates WHERE Date == '{date}' AND Type='LOCK';"
        curs = self.cursor.execute(sqlcmd)
        res = curs.fetchone()
        self.lockdate = res[0]
    ...
    def register_now_cal_date(self):
        ...
        """
        self.connect()
        sqlcmd = "INSERT OR IGNORE INTO RecDates (Date, Type, Giv_id) "\
                    f"VALUES (date('now'),'MEAS', '{self.giv_id}');"
        curs = self.cursor.execute(sqlcmd)
        self.connector.commit()
    ...
    def get_range_fullscale(self, range_key):
        self.connect()
        sqlcmd = f"SELECT fullscale FROM Ranges WHERE key_id = {range_key}"
        cur = self.cursor.execute(sqlcmd)
        fs = cur.fetchone()[0]
        return fs
    ...
    def register_measure(self, ref, meas, ko):
        ...
        """
        self.connect()
        sqlcmd = f"INSERT INTO Measure_points (Giv_ref, Start_key, Date_meas, Range_ref, Ref_val, Meas_val, Flg_ko) VALUES("\
                 f"{self.giv_key}, {self.start_key}, datetime('now','localtime'), {self.range_key}, {ref}, {meas}, {ko});"
    ...
    def get_dates_of_measures_for_registrered_Giv(self, only_last = False):
        ...
        """
        sqlcmd = f"SELECT DATE(Date_meas) as day FROM Measure_points "\
                 f"WHERE (Measure_points.Giv_ref == {self.giv_key}) "\
                 f"GROUP BY day ORDER BY Date_meas DESC;"
        print(sqlcmd)
        cur = self.cursor.execute(sqlcmd)        
    ...
    def get_ranges_of_measures_by_date_and_start_for_registrered_Giv(self, date, start_key = None):
        ...
        sqlcmd = "SELECT Range_ref, Range FROM Measure_points,Ranges WHERE "\
                 ...
                 f"AND Range_ref = Ranges.Key_id AND Range_ref = Ranges.Key_id " + sql_start_key + \
    ...
    def get_ranges_of_measure_by_giv_key_for_a_date(self, giv_key, date):
        ...
        sqlcmd = "SELECT Range_ref, Range FROM Measure_points, Ranges WHERE"\
                 f" Giv_ref = {giv_key} AND DATE(Date_meas) = '{date}' GROUP BY Range_ref"
        cur = self.cursor.execute(sqlcmd)
        rec_result = cur.fetchall()
        return (rec_result)  

    def get_all_givs_and_last_measures_dates(self):
        sqlcmd = "SELECT Key_id, Giv_id, MAX(Date(Date_meas)) AS mes_date FROM GIV_ids, Measure_points "\
                 "WHERE Giv_id IS NOT 'None' AND Key_id = Giv_Ref " \
                 " GROUP BY Key_id ORDER BY mes_date DESC;"
        cur = self.cursor.execute(sqlcmd)
        rec_result = cur.fetchall()
        return (rec_result)  
    ...
    def get_measures_by_range_date_giv_id(self, date, range_id, start_key=None):
        ...
        self.connect()
        sql_start_key = f" AND Start_key = {start_key}" if start_key != None else ""
         # 28/08-1       sqlcmd = "SELECT MAX(Date_meas), Range_ref, Ref_val, Meas_val FROM Measure_points WHERE "\
         # 28/08-2       sqlcmd = "SELECT DATE(MAX(Date_meas)), Range_ref, Ref_val, Meas_val FROM Measure_points WHERE "\

        sqlcmd = "SELECT MAX(Date_meas), Range_ref, Ref_val, Meas_val, Flg_ko FROM Measure_points WHERE "\
    ...
    def populate_database(self):
        ...
        for range, range_data in cfg_ranges.items():
            ...
            (tol, digit) = range_data['tolerance'] if 'tolerance' in range_data else (0.0, 0.0)
            ...
            if 'active' in range_data and range_data['active']:
                ...
                sql = "INSERT OR IGNORE INTO `Ranges` (Gamme, Range, To_calibrate, fullscale, tolerance, digit) VALUES"\
                      f" ( '{range}', '{range_data['english_name']}', {to_cal}, {fs}, {tol}, {digit} )"
